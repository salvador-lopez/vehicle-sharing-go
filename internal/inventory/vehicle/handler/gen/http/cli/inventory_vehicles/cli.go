...
import (
	...
	"vehicle-sharing-go/internal/inventory/vehicle/handler/gen/http/car/client"
)
...
func ParseEndpoint(
	...
	)
	{
		switch svcn {
		case "car":
			c := client.NewClient(scheme, host, doer, enc, dec, restore)
			switch epn {
			case "create":
				endpoint = c.Create()
				data, err = client.BuildCreatePayload(*carCreateBodyFlag)
			case "get":
				endpoint = c.Get()
				data, err = client.BuildGetPayload(*carGetIDFlag)
			}
		}
	}
	if err != nil {
